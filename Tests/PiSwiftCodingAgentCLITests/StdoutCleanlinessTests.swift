...
private enum PiExtensionSDKTestSupport {
    static let paths: ExtensionCompiler.SDKPaths? = {
        if let paths = ExtensionCompiler.resolveSDKPaths() {
            return paths
        }

        let sourceFile = URL(fileURLWithPath: #filePath)
        let tuiPackage = sourceFile
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let piSwiftPackage = tuiPackage
            .deletingLastPathComponent()
            .appendingPathComponent("PiSwift")

        if let paths = findSDKPaths(in: piSwiftPackage) {
            return paths
        }
        ...
        process.arguments = [
            "swift", "build",
            "--package-path", piSwiftPackage.path,
            "--product", "PiExtensionSDK",
        ]
        ...
        return findSDKPaths(in: piSwiftPackage)
    }()

    private static func findSDKPaths(in package: URL) -> ExtensionCompiler.SDKPaths? {
        let build = package.appendingPathComponent(".build")
        var candidates = [
            build.appendingPathComponent("out/Products/Debug"),
            build.appendingPathComponent("debug"),
        ]
        if let directories = try? FileManager.default.contentsOfDirectory(
            ...
            candidates += directories
                .filter { $0.lastPathComponent.hasSuffix("-apple-macosx") }
                .map { $0.appendingPathComponent("debug") }
        }

        for directory in candidates {
            let library = directory.appendingPathComponent("libPiExtensionSDK.dylib")
            guard FileManager.default.fileExists(atPath: library.path) else {
                continue
            }

            let modules = directory.appendingPathComponent("Modules")
            if FileManager.default.fileExists(atPath: modules.appendingPathComponent("PiExtensionSDK.swiftmodule").path) {
                return ExtensionCompiler.SDKPaths(modulePath: modules.path, libPath: directory.path)
            }
            if FileManager.default.fileExists(atPath: directory.appendingPathComponent("PiExtensionSDK.swiftmodule").path) {
                return ExtensionCompiler.SDKPaths(modulePath: directory.path, libPath: directory.path)
            }
        }
        return nil
    }
...
private func stdoutCleanlinessEnvironment(agentDir: String) -> [String: String] {
    ...
    if env["PI_EXTENSION_SDK_PATH"] == nil,
       let sdkPaths = ExtensionCompiler.resolveSDKPaths() ?? PiExtensionSDKTestSupport.paths {
